...
const Myorders = () => {
  ...
  const {
    ...
    fetch(`https://ancient-citadel-17819.herokuapp.com/order?email=${user.email}`, {
      method: "GET",
      headers: {
            ...
            {myOrder.map((mo, index) => (
              <tr key={mo._id}>
                ...
                <td>
                  ...
                  {mo.totalAmount && !mo.paid ? (
                    <Link to={`/dashboard/payment/${mo._id}`}>
                      <button className="btn btn-ghost bg-blue-500 hover:bg-blue-600 btn-xs text-white m-2">
                        Payment
                      </button>
                    </Link>
                  ) : (
